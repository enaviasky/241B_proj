...
\section{Results}
	\indent Figure X shows the power and area versus the number of bits approximated for the LPO adder. As the figure indicates, there is almost a linear relationship between the number of bits approximated versus power. Practically, this indicates that the optimal savings in power and area is dependent on how much the application can tolerate error. Figure X shows how performance can be optimized with number of bits. It is important to note however, that this frequency is an estimate obtained from how much positive slack was obtained given a fixed period. Synopsys Design Compiler would need to be re-run at the optimal slack to obtain power and area specifications at that frequency. In this paper, we chose to use a fixed period in order to compare power and area metrics across different implementations of the adder. (maybe include how many times power decreases with each bit or sth?) \\
	
	\indent Include accuracy plot here? \\
%	CNN accuracy

	\indent Figure X shows the power and area versus VBL of the approximate multiplier. As expected, power and area are reduced as the VBL is increased. What is interesting to note, however, is the estimated operating frequency (Figure X) barely changes when the VBL is under 30. Significant impact to the critical path only occurs at very high VBL values.
