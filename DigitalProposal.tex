...
\subsubsection{ASIC Implementation}
	\indent ASIC designs for varying approximate bit widths for the LPO adder as well as varying VBLs for multipliers have been implemented in the STM 28nm CMOS process (? correct process?). As a baseline, non-approximate versions of the adder and multiplier were implemented as well. Each design was compiled using DC and ICC synthesis, and power measurements were obtained using Synopsys PrimeTime. In order to obtain a comparable baseline, the clock periods for both the adder and multiplier blocks were fixed, and the obtained positive slack was used to estimate operating frequency. The resulting plots for power, performance, and area are shown in Fig. 1, 2, 3 and 4. \\
